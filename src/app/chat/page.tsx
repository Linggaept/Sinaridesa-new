...
import { useChat, ChatMessage as Message, Role } from "@/components/chat-context"
import Link from "next/link"
import ReactMarkdown from "react-markdown"
import remarkGfm from "remark-gfm"

export default function ChatPage() {
  const { active, isTyping, setSidebarOpen, sendUserMessage } = useChat()
        ...
        <header className="sticky top-0 z-10 bg-background/70 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-b">
          <div className="flex items-center gap-2 p-3 md:p-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-sm font-medium text-pretty">{active?.title || "Percakapan"}</div>
                <div className="text-xs text-muted-foreground">AI Sinari Desa</div>
              </div>
            </div>
          </div>
        </header>
        ...
        <form onSubmit={handleSend} className="p-3 md:p-4">
          <div className="flex items-center gap-2">
            ...
            <Input
              ...
              className="bg-muted"
            />
            <Button type="submit" className="bg-primary text-primary-foreground" disabled={!input.trim()}>
              Kirim
            </Button>
          </div>
        </form>
...
function ChatBubble({
  ...
  return (
    <motion.div
      ...
      className={cn("mb-3 md:mb-4 flex items-end gap-2", isUser ? "justify-end" : "justify-start")}
      role="listitem"
